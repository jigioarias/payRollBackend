...
module.exports = {
  async ingresar(request) {
    ...
    const diasNomina = (request.payload.classPayRoll.payrolltype=='J')?31:30;
    let valorHora = (request.payload.salary/diasNomina)/request.payload.classPayRoll.dayHours;
  ...
  //  console.log(request.payload.classPayRoll);
    
   const  guardarVacacion = await CreateVacacionEmpleado(enterprise, document, enjoyInitDate, enjoyEndDate,moneyDays, remuneration,user,employeeId,year, periodo,clase,enjoyDays,salary, { vacacionRepository});
   
   console.log('request.payload.vacationRequest>>>>>>>>>>>',request.payload.vacationRequest);
  ...
  //  console.log(periodoCLase);
    
  //const periodoCLaseAnterior = await getPeriodoAnterior(request.payload.period,request.payload.classPayRoll,{ periodClaseRepository });

  //console.log(periodoCLaseAnterior);
