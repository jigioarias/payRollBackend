...
module.exports = {
  ...
  async findEmpleados() {
    // Treatment
    const empleados = await ListEmpleados({  empleadoRepository });
    // Output
       
    const empleadoSerializer = new EmpleadoSerializer();
    const response =new Response(empleados.map(empleadoSerializer.serialize),'','');
    const responseSerializer = new ResponseSerializer();
    return responseSerializer.serialize(response);
