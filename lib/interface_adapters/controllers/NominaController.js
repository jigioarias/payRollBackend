...
const UpdateNomina = require('../../application_business_rules/use_cases/UpdateNomina');
...
const UpdateNovedadNominaEmpleado = require('../../application_business_rules/use_cases/UpdateNovedadNominaEmpleado');
...
const {ESTADO_PRESTAMO_PAGADO,ESTADO_PRESTAMO_PENDIENTE,DIAS_COMERCIAL,DIAS_JULIANO,CODIGO_DIAS_JULIANO,CODIGO_PENDIENTE,TIPO_AJUSTE_SUMA, TIPO_AJUSTE_RESTA,NOVEDAD_PENDIENTE, NOVEDAD_APROBADA, CODIGO_APROBADO} =require('./constantesSistema')
...
module.exports = {
  async createNomina(request) {
    ...
    //console.log('listGroupConceptos ',listGroupConceptos);   
      ...
      let removeNovedadesNomina = await RemoveNovedadNominaEmpleado(  empresaId,claseId,periodValue,NOVEDAD_PENDIENTE,{novedadNominaRepository});
  ...
  //actualizar Novedades Nomina
  let updateNovedadesNomina = await UpdateNovedadNominaEmpleado(  empresaId,claseId,periodValue,NOVEDAD_APROBADA,{novedadNominaRepository});

  // actualizar Nomina

  let updateNomina = await UpdateNomina( empresaId,claseId,periodValue,CODIGO_PENDIENTE,CODIGO_APROBADO, { nominaRepository },{detalleNominaRepository});

    //si todo esta OK devuelve la lista de empleados procesados con su respectivas novedades y conceptos
    response = new Response(updateNomina, 'OK', '');
      return responseSerializer.serialize(response);
