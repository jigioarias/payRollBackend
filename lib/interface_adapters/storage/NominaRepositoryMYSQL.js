...
module.exports = class {
  ...
  async persist(nominaEntity) {
    ...
    const {  enterprise,clase, employeeId, document, name,address,email,phone,period,concept,conceptName,valor,percentaje,conceptType,InitDayPay,endDayPay, type, user } = nominaEntity;
    
    const seqNomina = await this.model.create({  enterprise,clase, employeeId, document, name,address,email,phone,period,concept,conceptName,valor,percentaje,conceptType,InitDayPay,endDayPay,type,user });
    ...
    return new Nomina(seqNomina.id,  seqNomina.enterprise, seqNomina.clase, seqNomina.employeeId, seqNomina.document,seqNomina.name,
        seqNomina.address, seqNomina.email, seqNomina.phone, seqNomina.period, seqNomina.concept, seqNomina.conceptName, 
        seqNomina.valor,  seqNomina.percentaje,  seqNomina.conceptType, seqNomina.InitDayPay,seqNomina.endDayPay, seqNomina.type, seqNomina.user);
  }
  ...
  async merge(nominaEntity) {
    ...
    const {  enterprise,clase, employeeId, document, name,address,email,phone,period,concept,conceptName,valor,percentaje,conceptType,InitDayPay,endDayPay, type, user } = nominaEntity;
    await seqconcepto.update({ enterprise,clase, employeeId, document, name,address,email,phone,period,concept,conceptName,valor,percentaje,conceptType,InitDayPay,endDayPay, type, user });
