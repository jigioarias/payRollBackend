...
module.exports = class {
    ...
    const seqEmpleados = await this.model.findAll(
      {
         ...
         where: {active: empleado.active,
                enterprise:empleado.enterprise,
                classPayRoll:empleado.classPayRoll
                }}
     );
  ...
  async getById(idEmployee) {

    const seqEmpleado = await this.model.findByPk(idEmployee);
  ...
  async merge(empleadoEntry) {
    ...
    console.log('empleado ENTRYZZZZ',empleadoEntry);
    const { enterprise, idPerson, salary, salaryType,initDateContract,endDateContract, costCenter, classPayRoll, departament,branchOffice,unity,area,active,user ,transporteSubsidy} = empleadoEntry;
  
    let seqEmpleado = await this.model.findOne({ where: { id: empleadoEntry.id } });
