...
async find(nominaEntity) {
  ...
  async findByEmpleadoPeriod(nominaEntity) {

    const seqPrestaciones = await this.model.findAll({ 
                                                    attributes: [ [sequelize.fn('sum', sequelize.col('valor')), 'totalVacaciones']],
                                                    where: { enterprise: nominaEntity.enterprise, 
                                                      type:nominaEntity.type,
                                                      document:nominaEntity.document
