...
module.exports = class {
  ...
  async persist(licenciaEntity) {
    const {  enterprise, document, initDate, endDate,  type ,remuneration,user,employeeId,year, registerPeriod,clase,salary,active } = licenciaEntity;
    console.log(licenciaEntity.active);
    const seqLicencia = await this.model.create({  enterprise, document, initDate, endDate,  type ,remuneration,user,employeeId, year, registerPeriod,clase,salary,active });
    ...
    return new Licencia(
      ...
      seqLicencia.salary,seqLicencia.active);
  }
